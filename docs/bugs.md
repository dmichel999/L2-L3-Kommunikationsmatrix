...
### MAC-Adressen-Ansicht zeigte über Port-Channel gelernte Adressen fälschlich als lokal (behoben 2026-08-19)
**Symptom:** Feature 3 (Klick auf VLAN → MAC-Adressen-Ansicht) sollte laut Doku Uplinks zwischen Switches ausschließen, zeigte aber trotzdem MAC-Adressen, die ein Switch nur über einen Port-Channel-Uplink zu einem anderen Switch gelernt hatte — als wären es lokale Endgeräte an diesem Switch.
**Ursache:** `computeUplinkPortsBySwitch()` (`js/core/mac-model.js`) markiert bei einem erkannten CDP-Uplink zwar alle physischen Member-Ports des zugehörigen Port-Channels als Uplink, nicht aber die Port-Channel-Schnittstelle selbst (z.B. `Po1`). `show mac address-table` weist eine über ein Bündel gelernte MAC-Adresse aber genau dieser Schnittstelle zu, nicht einem einzelnen physischen Member — der Abgleich griff deshalb nie.
**Fix:** Beim Erkennen eines Uplink-Port-Channels wird jetzt zusätzlich die normalisierte Port-Channel-ID selbst (`pc.portChannelId`) in die Uplink-Menge aufgenommen. Betrifft neben der MAC-Ansicht auch die Duplicate-Erkennung (Feature 21), die dieselbe Funktion nutzt (dort ließ derselbe Bug MAC-Adressen fälschlich als Duplikat über zwei Switches hinweg erscheinen).
**Gefunden durch:** Nutzer-Hinweis; per Unit-Test (JXA-Fixture mit zwei über einen 2-Member-Port-Channel verbundenen Switches) verifiziert.
